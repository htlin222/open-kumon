...
`src/content/kyoiku-revision-2020.ts` 裡的 `MOVED` 陣列，**必須**對照下列任一權威來源逐字核對後填入：

- 文部科学省「小学校学習指導要領（平成29年告示）別表 学年別漢字配当表」
- 或 Wikipedia「教育漢字」條目的現行年級表

Task 1 的計數測試就是這份 patch 的驗收條件 —— 填錯，counts 對不上，測試會紅。**不要為了讓測試變綠而去改 `GRADE_KANJI_COUNTS`**；那組數字是文部科学省定的，是事實不是設定。
