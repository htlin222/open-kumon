...
const g4 = JSON.parse(readFileSync('content/kanji/4.json', 'utf8')) as KanjiEntry[]
const g5 = JSON.parse(readFileSync('content/kanji/5.json', 'utf8')) as KanjiEntry[]

/**
 * 覆蓋率門檻分年級。1 年級幾乎都是具象字（日月火水山川），2 年級開始大量
 * 抽象字（元・公・当・番・週），硬湊圖只會誤導孩子，所以門檻本來就該降。
 */
// 門檻逐年遞減不是妥協，是題材：高年級的字（抽象詞：制・則・務・態）
// 本來就畫不出來，硬套圖會誤導。背面看圖寫字只需要 10 個字，這個量始終足夠。
const MIN_COVERAGE = { 1: 0.7, 2: 0.35, 3: 0.3, 4: 0.18, 5: 0.15 } as const
...
describe('OpenMoji 意味插圖', () => {
  ...
  it.each([
    ...
    [4, g4],
    [5, g5],
  ])('%i 年級覆蓋率達門檻', (grade, list) => {
    const hit = list.filter((e) => e.openmoji).length
    expect(hit / list.length).toBeGreaterThanOrEqual(MIN_COVERAGE[grade as 3 | 4 | 5])
  })

  it('背面 10 題所需的插圖字，每個年級都足夠', () => {
    for (const [grade, list] of [[1, g1], [2, g2], [3, g3], [4, g4], [5, g5]] as const) {
  ...
  it('對照表裡的字都在已建構的年級配当表內', () => {
    const chars = new Set([...g1, ...g2, ...g3, ...g4, ...g5].map((e) => e.kanji))
    for (const kanji of Object.keys(map)) {
      expect(chars.has(kanji), `「${kanji}」不在 1–5 年級`).toBe(true)
    }
  })

  it('產出的 openmoji 欄位與對照表一致', () => {
    for (const e of [...g1, ...g2, ...g3, ...g4, ...g5]) {
