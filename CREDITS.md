# CREDITS

本專案使用下列公開資源。各項授權條款以原始出處為準。

## 資料

| 素材 | 出處 | 授權 | 用途 |
|---|---|---|---|
| KANJIDIC2 | [EDRDG](http://www.edrdg.org/wiki/index.php/KANJIDIC_Project) | CC BY-SA 4.0 | 音読み・訓読み・画数 |
| kanjiapi.dev | [kanjiapi.dev](https://kanjiapi.dev/) | — （資料源自 KANJIDIC2 / JMdict） | 上述資料的 JSON 介面 |
| KanjiVG | [KanjiVG](https://kanjivg.tagaini.net/) | CC BY-SA 3.0 | 筆順路徑、なぞり書き範字 |
| JMdict | [EDRDG](https://www.edrdg.org/jmdict/j_jmdict.html) | CC BY-SA 4.0 | 語彙、讀音 |
| JLPT 分級詞表 | [Jonathan Waller / tanos.co.uk](https://www.tanos.co.uk/jlpt/)（經 [Bluskyo/JLPT_Vocabulary](https://github.com/Bluskyo/JLPT_Vocabulary) 轉為 JSON） | CC BY | 語彙的難易度排序。**非官方** —— JLPT 自 2010 年改制後不再公布官方詞表，這是依舊制重建的通行版本 |
| Tatoeba | [tatoeba.org](https://tatoeba.org/) | CC BY 2.0 FR | 例句（經 JMdict 收錄） |
| 学年別漢字配当表 | 文部科学省 小学校学習指導要領（平成29年告示）別表 | 公開資料 | 年級與字的對應（令和2年度實施版） |

## 圖像

| 素材 | 出處 | 授權 | 用途 |
|---|---|---|---|
| OpenMoji | [openmoji.org](https://openmoji.org/) | CC BY-SA 4.0 | 意味插圖（black 線稿版） |
| ~~Open Peeps~~ | [openpeeps.com](https://www.openpeeps.com/) | CC0 | **未採用** —— 沒有可程式化取得的發布，官網只提供 Sketch/Figma 檔手動下載 |
| ~~Humaaans~~ | [humaaans.com](https://www.humaaans.com/) | CC BY 4.0 | **未採用** —— `react-humaaans` 是 2019 年的 UMD bundle，ESM 下沒有可用的 default export |
| 陪伴角色 | 本專案自繪（`src/components/Mascot.tsx`） | MIT | 螢幕與紙面共用的線稿角色 |
| Phosphor Icons | [phosphoricons.com](https://phosphoricons.com/) | MIT | 孩子看的大圖示 |
| Tabler Icons | [tabler.io/icons](https://tabler.io/icons) | MIT | App 外殼圖示 |
| Lucide | [lucide.dev](https://lucide.dev/) | ISC | 補缺圖示 |

## 字型

| 字型 | 出處 | 授權 | 用途 |
|---|---|---|---|
| Klee One | [Fontworks](https://fonts.google.com/specimen/Klee+One) | SIL Open Font License 1.1 | 練習單日文範字（教科書体風） |

## 說明

OpenMoji 與 KanjiVG 皆為 **CC BY-SA**，share-alike 具傳染性。因此本專案 `content/` 目錄下的產出一併以 **CC BY-SA 4.0** 釋出。程式碼本身為 MIT。

課程骨架參考「くもんの小学ドリル 国語」的系列結構。本專案與 くもん出版 / 公文教育研究会 **無任何關係**，未使用其題目、版面或商標。
